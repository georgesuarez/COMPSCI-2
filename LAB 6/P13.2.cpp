/*
        ...
        that compute the set union and intersection of the sets a and b. (Don�t
   name the first function union�that is a reserved word in C++.)
*/
#include <iostream>
#include <set>
using namespace std;
...
int main() {
  ...
  cout << "Set union: ";
  for (set<int>::iterator iter = new_set.begin(); iter != new_set.end();
       iter++) {
    cout << *iter << " ";
  }
  ...
  for (set<int>::iterator iter = new_set2.begin(); iter != new_set2.end();
       iter++) {
    cout << *iter << " ";
  }
