/*
        ...
        Checking. In the base class, supply member functions deposit and
   withdraw. Provide a function daily_interest that computes and adds the daily
   interest. For calculations, assume that every month has 30 days. Checking
   accounts yield interest of 3 percent monthly on balances over $1,000. Savings
   accounts yield interest of 6 percent on the entire balance. Write a driver
   program that makes a month�s worth of deposits and withdrawals and calculates
   the interest every day.
*/
...
Account::Account() { amount = 0.0; }

Account::Account(double new_amount) { amount = new_amount; }
...
void Account::set_deposit(double new_amount) { amount += new_amount; }

void Account::set_withdrawl(double new_amount) { amount -= new_amount; }

double Account::daily_interest() { return 0; }
...
int main() {
  ...
  if (more) {
    cout
        << "Enter the amount days you would like to make deposits/withdrawls: ";
    cin >> days;
    cout << endl;
  ...
  while (days > 0) {
    ...
    cout << "Daily interest earned: $" << fixed << setprecision(2)
         << account1->daily_interest() << endl;
    cout << "Remaing balance: $" << fixed << setprecision(2)
         << account1->get_amount() << endl;
    cout << endl;
    n++;
    days--;
  }
