/**
        ...
        database. The template file contains text and tags. The tags have the
form |1| |2| |3|� and need to be replaced with the first, second, third, � field
...
And here is a typical form letter:
        ...
        Dear |1| |3|:
        You and the |3| family may be the lucky winners of $10,000,000 in the
C++ compiler clearinghouse sweepstakes! ...
*/
#include <fstream>
#include <iostream>
#include <sstream>
...
int main() {
  ...
  while (getline(inFile, more_data)) {
    ...
    if (pos != string::npos) {
      more_data.replace(pos, more_data.length(),
    ...
    if (pos3 != string::npos) {
      more_data.replace(pos3, more_data.length(),
                        info[4] + ", " + info[5] + " " + info[6]);
    }

    size_t pos4 = more_data.find("Dear |1| |3|:");
    if (pos4 != string::npos) {
      more_data.replace(pos4, more_data.length(),
                        "Dear " + info[0] + " " + info[2]);
    }
    size_t pos5 = more_data.find("You and the |3| family may be the lucky "
                                 "winners of $10,000,000 in the C++");
    if (pos5 != string::npos) {
      more_data.replace(
          pos5, more_data.length(),
          "You and the " + info[2] +
              " family may be the lucky winners of $10,000,000 in the C++");
    }
