/*
    ...
    Change the stream output operator so that it will produce a special marker,
        *****, when such a value is printed. Modify the conversion operator
   double so that it will produce the value zero in this case. Finally, add a
   conversion operator bool so as to test whether a Fraction is proper.
*/
